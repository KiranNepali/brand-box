...
function BlogCardsTutorials({}: Props) {
  return (
    <div className="w-full mt-[10rem]">
      {/* title  */}
      <div className="w-full flex justify-between items-center">
        <span className="font-semibold text-xl">Tutorials</span>
        {/* button  */}
        <button className="px-2 py-1 hover:text-primary-50 hover:bg-secondary-300 duration-150  border-[1px] rounded-lg text-sm font-medium border-primary-200">
          View all
        </button>
      </div>
      <div className="w-full">
        <div className="flex flex-col gap-10 mt-5">
          {BlogData.blogTutorialsData.map((item, index) => (
            <div
              key={index}
...
export default BlogCardsTutorials;
